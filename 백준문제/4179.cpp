...
int moveJihoon(int a, int b) {
	...
	while(!Q.empty()) {
			...
			for(int dir=0; dir<4; dir++) {
				...
				if(ny<0 || ny>=R || nx<0 || nx>=C) return map[y][x] + 1;
				if(visited[ny][nx] || map[ny][nx] == -1) continue;
				if(fireMap[ny][nx] >=0 && fireMap[ny][nx] <= map[y][x] + 1) continue;
				visited[ny][nx] = true;
				map[ny][nx] = map[y][x] + 1;
...
int main(void) {
	...
	string str;
	cin >> R >> C;
	for(int y=0; y<R; y++) fill(fireMap[y], fireMap[y]+C, INF);
	...
	for(int y=0; y<R; y++) fill(visited[y], visited[y] + C, false);
	while(!Q.empty()) Q.pop();
	Answer = moveJihoon(Jihoon.first, Jihoon.second);
